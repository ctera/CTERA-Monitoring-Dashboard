...
- Portal login fails with HTTP 403 / `Authentication failed` / `portalErrorPage` while browser login works: confirm the monitoring server IP is allowed under **Settings → Control Panel → Global Administrators Access Control**, and that the stored user is a Global Admin with a local password (especially when SAML SSO is enabled).
